	...
	$.ajax({
			...
			success:function(data){
				...
				$.each(data,function(index,json){
					html += '<tr class="pointer mouse_hover" onclick="change_goods(\''+json.t_code+'\',\''+json.goods_code+'\',\''+json.b_repo+'\')\"><td>'+json.ss_type+'</td><td>'+json.t_code+'</td><td>'+json.goods_code+'</td><td>'+json.b_repo+'</td><td>'+json.b_warning+'</td></tr>';
				})
				  html = '<tr class="title_bg"><td>分类</td><td style="width:200px;">条形码</td><td style="width:260px;">商品代码</td><td style="width:100px;">库存</td><td style="width:100px;">阈值</td></tr>'+html;
				$("#page_data").html(html);
...
function inner_change_b_repo(){
	...
	if(c_b_repo==""){
		...
		$.ajax({
			...
			data:"o_code="+encodeURIComponent(o_code)+"&c_b_repo="+encodeURIComponent(c_b_repo),
			success:function(data){
				if(data=="ok"){
...
<div class="all_num f_right">【合计】<span id="all_num">{$all_num}</span> 个</div>
...
<div class="inner_alert">
    ...
    <input type="hidden" id="o_code" name="o_code" value="" />
        ...
        <li><span class="title_list">库存调整</span> <input class="input_key" id="c_b_repo" type="text" value="" name="c_b_repo" /></li>
    </ul>
    <div class="clear"></div>
    <div class="ok_bar" style="width:210px;">
        <div class="btn_green f_left border_green" style="width:80px !important;" onClick="inner_change_b_repo()">确 定</div>
