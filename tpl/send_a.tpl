  ...
  $.ajax({
    ...
    success:function(data){
      ...
      $.each(data,function(index,json){
        ...
        ycmbcd+='<li class="can_use" onclick="send_key(\''+json.t_code+'\',\''+json.goods_code+'\',\''+json.a_repo+'\')">'+json.t_code+'</li>';
      })
...
function autosearch(){
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      $.each(data,function(index,json){
        //alert(json.goods_code.length())
        ycmbcd+='<li class="can_use" onclick="send_key(\''+json.t_code+'\',\''+json.goods_code+'\',\''+json.a_repo+'\')">'+json.goods_code+'</li>';
      })
      $("#key_autosearch").html(ycmbcd);
    }
  })  
...
function check_a(){
	...
	$.ajax({
		url:"send_1.php",
		type:"GET",
		data:"check_a="+out_num+"&goods_code="+encodeURIComponent(goods_code),
		success:function(data){
			if(data=="no"){
				 $(".txt_msg").html("库存不足！");
				 $(".msg").stop(true,true).show(200).delay(1000).fadeOut(200);
...
<div id="repo_box">中国库存：<span id="repo">*</span></div>
<div class="add_list">
	<ul>
...
<div class="add_list">
  <ul>
  	  <li><span class="title_list">出库数目</span> <input class="add_listkey" onBlur="check_a()" type="text" placeholder="必须输入..." name="out_num" id="out_num" /></li>
